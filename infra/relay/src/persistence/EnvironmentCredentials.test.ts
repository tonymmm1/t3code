...
describe("EnvironmentCredentials", () => {
  it.effect("creates opaque environment credentials and stores only the hash", () => {
    ...
    return Effect.gen(function* () {
      ...
      expect(query.sql).not.toContain('"relay_environment_credentials"."environment_public_key"');
      expect(query.sql).toContain('"relay_environment_credentials"."credential_id" <> $2');
      expect(query.sql).toContain('"relay_environment_credentials"."revoked_at" is null');
      expect(query.params).toEqual(["env_test", credentialId]);
    }).pipe(
      Effect.provide(
