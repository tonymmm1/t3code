...
import {
  AgentAwarenessRelay,
  type AgentAwarenessRelayShape,
} from "../Services/AgentAwarenessRelay.ts";
...
const make = Effect.gen(function* () {
  ...
  const start: AgentAwarenessRelayShape["start"] = Effect.fn("start")(function* () {
    const relayConfig = yield* readRelayConfig.pipe(Effect.orElseSucceed(() => null));
    if (!relayConfig) {
    ...
    yield* Effect.forkScoped(
      Stream.runForEach(orchestrationEngine.streamDomainEvents, (event) => {
        ...
        if (!shouldPublishAgentAwarenessEvent(event)) {
          return Effect.logDebug("agent awareness relay ignored event without awareness changes", {
            eventType: event.type,
            threadId,
          });
        }
...
export const AgentAwarenessRelayLive = Layer.effect(AgentAwarenessRelay, make);
