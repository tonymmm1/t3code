...
import { AgentAwarenessRelay } from "../Services/AgentAwarenessRelay.ts";
import {
  AgentAwarenessRelayLive,
...
function makeMemorySecretStore() {
  ...
  const store = {
    get: ((name) =>
      Effect.sync(() => values.get(name) ?? null)) satisfies ServerSecretStoreShape["get"],
    set: ((name, value) =>
      Effect.sync(() => {
        values.set(name, Uint8Array.from(value));
      })) satisfies ServerSecretStoreShape["set"],
    create: ((name, value) =>
      Effect.sync(() => {
        values.set(name, Uint8Array.from(value));
      })) satisfies ServerSecretStoreShape["create"],
    getOrCreateRandom: ((name, bytes) =>
      Effect.sync(() => {
        ...
        return generated;
      })) satisfies ServerSecretStoreShape["getOrCreateRandom"],
    remove: ((name) =>
      Effect.sync(() => {
        values.delete(name);
      })) satisfies ServerSecretStoreShape["remove"],
...
describe.sequential("signRelayAgentActivityPublishProof", () => {
  ...
  it("deduplicates awareness state updates whose only change is their event timestamp", () => {
    expect(agentAwarenessPublishIdentity(state)).toBe(
      agentAwarenessPublishIdentity({
        ...state,
        updatedAt: "2026-05-25T00:10:00.000Z",
      }),
    );
    expect(agentAwarenessPublishIdentity(state)).not.toBe(
      agentAwarenessPublishIdentity({
        ...state,
        phase: "completed",
